/**
 * Deploy the Supply Chain Tracker contract to a Midnight network
 * (undeployed/local devnet by default; use --network preview|preprod for public networks).
 *
...
import { createWitnesses, type PrivateState } from './witnesses';
...
// Identifier under which this contract's private state is stored. This
// contract keeps no persistent private ledger state, so it is the empty {}.
...
// Witnesses are bound at compile time. During deployment only the constructor
// runs (no witnesses invoked), so neutral all-zero secrets are safe here.
const compiledContract = CompiledContract.make(
  'supply-chain',
  SupplyChain.Contract as unknown as typeof SupplyChainContract,
).pipe(
  CompiledContract.withWitnesses(createWitnesses({ batchSecret: new Uint8Array(32), handoffSecret: new Uint8Array(32) })),
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy supply-chain-tracker to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
